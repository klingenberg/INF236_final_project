...
In figure \ref{fig:cutoff_levels}, the difference in the speedup of the both dimensions at the cutoff level is not that large, so we did not recompute our results (also because it takes a very long time to execute several runs of the algorithms). With an optimal submatrix dimension size of 64 $\times$ 64, it would be possible to achieve even more speedup with the parallel algorithms. \\
\\
