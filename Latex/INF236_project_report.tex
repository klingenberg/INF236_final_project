...
described in previous section with \texttt{\#pragma omp parallel for}.
...
but only 7 multiplications half-size matrices. Then, the algorithm is used recursively
...
Because of that, it is useful to use some recursive ordering for storing matrices in memory,
for example z-ordering (figures \ref{z_diagram}, \ref{z_mem}).
...
However, there is switch from Strassen's algorithm to the simple matrix multiplication
...
in row-wise order (figure \ref{z_partly}). Functions for reordering matrices to this layout
...
We decided to try some number of recursive steps and than switch to the parallelized simple multiplication.
...
After some measurements with 2-layers parallel Strassen's algorithm, we implemented this version.
The idea is same, but there are more recursive calls of Strassen's algorithm with parallelized
additions and subtractions (similar to \texttt{parallel\_strassen\_level\_1}). When is reached
cutoff matrix size, it is switched to the parallelized simple multiplication. We tried some cutoffs
and we think that the best one is 512$\times$512.

Since there are at least two layers of Strassen's algorithm and we set the cutoff to 512$\times$512,
we need to solve cases when the matrices are smaller than 2048$\times$2048. We use only the
the parallelized simple multiplication in these cases.

\section{Experiments}

We run all tests on brake.ii.uib.no machine.
